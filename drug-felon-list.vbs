...
END IF
ObjExcel.Cells(1, col_to_use).Value = "DFLN Info"
objExcel.Cells(1, col_to_use).Font.Bold = TRUE
DFLN_col = col_to_use
col_to_use = col_to_use + 1
...
'Reset Excel Row
	...
	'This loop will navigate to DFLN and check if it exists.
	Do while case_number <> ""
		...
		'Check MFIP ELIG first'
		call navigate_to_MAXIS_screen("ELIG", "MFPR")
		EMReadScreen mfsc_check, 4, 3, 38 'getting past sig change screen'
		IF mfsc_check = "MFSC" Then Transmit
		row = 7
		DO
			DO
				EMReadScreen elig_status, 4, row, 53
				IF elig_status = "ELIG" Then
					EMReadScreen elig_member, 2, row, 6
					member_list = member_list & elig_member & ","
				END IF
				row = row + 1
			LOOP Until row = 20
			PF8
			EMReadScreen screens_check, 7, 24, 5
		Loop Until screens_check = "NO MORE"
		member_array = split(member_list, ",")
		'perhaps add all membs for all progs into the list and then remove duplicates and split'
		For each member in member_array


		ObjExcel.Cells(excel_row, DFLN_col).Value = dfln_info
		END IF
